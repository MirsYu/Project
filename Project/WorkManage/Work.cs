...
namespace Project
{
	public class Work
	{
        ...
        */
		...
		/// 工位管理，主要用于添加，初始化 
		/// </summary>
		public WorkObjectManager tag_WorkObjectManager;
		/// <summary>
		/// 工作工位列表，集中管理
		/// </summary>
		...
		public Work()
		{
			...
			tag_WorkObjectManager = new WorkObjectManager(this, tag_workObject);
			Init();
			IoCheckThreadStart();
		...
		public short Continue(object o1)
		{
			...
			{
				LogOutControl.OutLog("没有复位，请复位", 0);
				return -1;
			}
		...
		public bool start()
		{
			...
			{
				LogOutControl.OutLog("复位中，请等待", 0);
				return false;
			}
			if (Global.WorkVar.tag_ResetState == 0)
			{
			...
			{
				LogOutControl.OutLog("请到主界面按启动键", 0);
				return false;
			}
			if (Global.WorkVar.tag_StopState == 1)
			{

				LogOutControl.OutLog("急停中", 0);
				return false;
			}
			if (IsWork())
			{
				LogOutControl.OutLog("当前有工位正在工作，请停止正在工作的工位", 0);
				return false;
			}
