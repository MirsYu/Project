...
namespace Project
{
	class adt8940a1m
	{
		...
		/************************************************
		功能：初始化运动控制卡
		(1)返回值>0时，表示adt8940a1卡的数量。如果为3，则下
		   面的可用卡号分别为0、1、2；
		(2)返回值=0时，说明没有安装adt8940a1卡；
		...
		************************************************/
		...
		[DllImport("8940a1m.dll")]
		public static extern Int32 adt8940a1_get_card_index(Int32[] m_cardNum);
		/************************************************
		功能：获取当前所有8940A1卡的拨码开关值
		参数:
			index      拨码开关数值组成的数组，可用运动控制卡数量和本数组的元素个数保持一致
			index[0]   第1张控制卡拨码开关值
			index[1]   第2张控制卡拨码开关值
			index[2]   第3张控制卡拨码开关值
			...        最多支持的控制卡数量为10
		返回值   0：正确          1：错误
		注意：只有当PCI插槽中8940A1卡都带有拨码开关，并且其硬件版本都不低于3.0，才能通过拨码值设置卡号;
			否则将不启用拨码开关,初始化卡时会根据PCI插槽距离CPU的远近，自动分配卡号，卡号的顺序从0,1,2...依次增加。
			例如：如果3张8940A1卡全部带有拔码开关，且硬件版本为3.0，可以设置3个不同的拨码值为各张卡的卡号。
			如果1张8940A1卡带拔码开关，硬件版本为3.0，同时有其他不带拔码开关的8940A1卡一起使用时，不启用拨码开关。
			如果1张8940A1卡带拔码开关，硬件版本为2.6(低于3.0)，不能启用拨码开关。
		************************************************/
