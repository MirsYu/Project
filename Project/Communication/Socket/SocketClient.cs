...
namespace Project
{
	public class SocketClient
	{
		...
		public void Connect()
		{
			try
			{
				IPAddress _ip = IPAddress.Parse(tag_IPConfig.tag_ip);
				tag_IPEndPoint = new IPEndPoint(_ip, tag_IPConfig.tag_port);
			...
			{
				LogOutControl.OutLog(msg.Message, 0);
			}
		}
		/// <summary>
		/// 读服务器下发的数据,几台通信
		...
		public string Read(int outTime)
		{
			try
			{
				if (tag_Socket.Connected)
				{
					...
					{
						LogOutControl.OutLog(exx.Message, 0);
						return "";
					}
